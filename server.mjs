...
// Importar agente HTTP/HTTPS de Node.js
import { Agent as HttpsAgent } from "https";
// Importación CRÍTICA para solucionar el error de conexión en Fly.io
import { Client as FayeWebSocket } from "faye-websocket"; 
...
const createAndConnectSocket = async (sessionId) => {
  ...
  const { state, saveCreds } = await useMultiFileAuthState(sessionDir);
    ...
    // Agente HTTP/HTTPS para mantener la conexión activa
  ...
  const sock = makeWASocket({
    ...
    // ******* CORRECCIÓN CRÍTICA: Reemplazar el constructor de WebSocket *******
    // Esto fuerza a usar un cliente WebSocket más robusto para entornos como Fly.io
    generateHighQualityLink: true,
    shouldUse3p: true, // Esto también puede ayudar con los errores 405
    webSocket: FayeWebSocket // <--- Usar FayeWebSocket en lugar del ws nativo de Node
  });
